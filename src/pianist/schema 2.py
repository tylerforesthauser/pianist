...
class NoteEvent(BaseModel):
    ...
    def _normalize_pitch_fields(cls, data: Any) -> Any:
        ...
        if not (has_legacy or has_notes or has_groups):
            raise ValueError("One of 'pitch', 'pitches', 'notes', or 'groups' must be provided.")

        if sum([has_legacy, has_notes, has_groups]) > 1:
            raise ValueError("Provide only one of legacy 'pitch'/'pitches', 'notes', or 'groups'.")
        ...
        if pitch is not None:
            ...
            for idx, g in enumerate(groups):
                ...
                elif hasattr(g, "pitches"):
                    group_pitches = g.pitches
                elif hasattr(g, "pitch"):
                    # Convert singular "pitch" to list format
                    group_pitches = [g.pitch]
                else:
                    raise ValueError(
...
class TempoEvent(BaseModel):
    ...
    def _validate_tempo_fields(self) -> TempoEvent:
        ...
        has_gradual = (
            self.start_bpm is not None or self.end_bpm is not None or self.duration is not None
        )
...
Event = Annotated[NoteEvent | PedalEvent | TempoEvent | SectionEvent, Field(discriminator="type")]
...
class KeyIdea(BaseModel):
    ...
    importance: Literal["high", "medium", "low"] = Field(
        default="medium", description="Importance level for preservation/development"
    )
    development_direction: str | None = Field(
        default=None, description="How to develop (e.g., 'expand and vary', 'preserve exactly')"
    )
...
class ExpansionPoint(BaseModel):
    ...
    development_strategy: str = Field(
        description="How to expand (e.g., 'develop motif_1 with variations')"
    )
    preserve: list[str] = Field(
        default_factory=list, description="List of key_idea IDs to preserve during expansion"
    )
...
class MusicalIntent(BaseModel):
    ...
    key_ideas: list[KeyIdea] = Field(
        default_factory=list, description="List of key musical ideas (motifs, phrases, etc.)"
    )
    expansion_points: list[ExpansionPoint] = Field(
        default_factory=list, description="Points where the composition should be expanded"
    )
    preserve: list[str] = Field(
    ...
    development_direction: str | None = Field(
        default=None, description="Overall development direction for the composition"
    )

...
def generate_expansion_strategy(
    ...
    target_length: float,
    analysis: MusicalAnalysis | None = None
) -> ExpansionStrategy:
    """
    ...
    for i, motif in enumerate(analysis.motifs):
        ...
        motif_developments.append(MotifDevelopment(
    ...
    if composition.musical_intent and composition.musical_intent.expansion_points:
        for point in composition.musical_intent.expansion_points:
            section_ratio = point.suggested_length / point.current_length if point.current_length > 0 else 1.0
            techniques = _suggest_section_techniques(point, section_ratio)
            section_expansions.append(SectionExpansion(
                ...
                techniques=techniques,
                description=point.development_strategy or f"Expand section {point.section} using: {', '.join(techniques)}"
            ))
    else:
        ...
        if analysis.phrases:
            ...
            target_beats_per_phrase = target_length / len(analysis.phrases) if analysis.phrases else beats_per_phrase * expansion_ratio
            
            for i, phrase in enumerate(analysis.phrases):
                phrase_ratio = target_beats_per_phrase / beats_per_phrase if beats_per_phrase > 0 else expansion_ratio
...
def _generate_overall_approach(
    ...
    if expansion_ratio < 1.5:
        ...
        parts.append("Significant expansion: develop motifs extensively, add new sections, and create transitions")
    ...
    if composition.musical_intent and composition.musical_intent.development_direction:
        parts.append(f"Follow overall direction: {composition.musical_intent.development_direction}")
...
def suggest_motif_development(motif: Any, target_length: float) -> str:
    ...
    """
    expansion_ratio = target_length / (motif.start + motif.duration) if (motif.start + motif.duration) > 0 else 1.0
    techniques = _suggest_motif_techniques(motif, expansion_ratio)
...
def suggest_phrase_extension(phrase: Any, target_length: float) -> str:
    ...
    """
    expansion_ratio = target_length / (phrase.start + phrase.duration) if (phrase.start + phrase.duration) > 0 else 1.0

...
class TempoPoint(BaseModel):
    ...
    bpm: float = Field(
        ge=5, lt=400
    )  # Lowered from 20 to 5 to handle slow tempos (with clamping in analyze_midi)
...
def analyze_midi(path: str | Path) -> MidiAnalysis:
    ...
    for tr in mid.tracks:
        ...
        active_pedal: dict[int, tuple[int, int] | None] = dict.fromkeys(range(16))
    ...
    for ch in channels:
        ...
        pedal_ratio = float(pedal_ticks_covered / end_tick_global) if end_tick_global > 0 else None
        ...
        if onset_to_pitches:
            ...
            for a, b in itertools.pairwise(melody):
                melodic_intervals[int(b) - int(a)] += 1
        melodic_interval_hist = {str(k): int(v) for k, v in melodic_intervals.most_common(12)}
        ...
        track_analyses.append(
            TrackAnalysis(
                ...
                chord_size=Distribution(
                    ...
                    p25=_safe_quantile([float(x) for x in chord_sizes], 0.25)
                    if chord_sizes
                    else None,
                    ...
                    p75=_safe_quantile([float(x) for x in chord_sizes], 0.75)
                    if chord_sizes
                    else None,
    ...
    ts_note = f"{ts_points[0].numerator}/{ts_points[0].denominator}" if ts_points else "unknown"
    ...
    if primary is not None:
        ...
        if primary.chord_size.median is not None:
            texture_lines.append(
                f"- typical chord size (median): {primary.chord_size.median:.1f} notes"
            )

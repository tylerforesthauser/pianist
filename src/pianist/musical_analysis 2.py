...
def _analyze_chord_function(
    chord_obj: chord.Chord, key_obj: key.Key | None
...
def _detect_cadences(
    chords: list[ChordAnalysis], key_obj: key.Key | None
...
def analyze_harmony(
    composition: Composition, music21_stream: stream.Stream | None = None
) -> HarmonicAnalysis:
    """
    ...
    s = _composition_to_music21_stream(composition) if music21_stream is None else music21_stream
        ...
        for element in part.flatten().notes:
            if hasattr(element, "offset"):
                ...
                if chord_obj and chord_pitches:
                    ...
                    roman_num, function, inversion = _analyze_chord_function(
                        chord_obj, detected_key_obj
                    )

                    chord_analyses.append(
                        ChordAnalysis(
                            start=offset,
            ...
            if len(group) >= 2:  # Pattern appears at least twice
                ...
                if min_length <= duration <= max_length:
                    ...
                    for existing_motif in motifs:
                        if (
                            abs(existing_motif.start - first_occurrence) < 1.0
                            and abs(existing_motif.duration - duration) < 1.0
                        ):
                            is_duplicate = True
                            break
...
def _calculate_section_similarity(
    section1_notes: list[tuple[float, int]], section2_notes: list[tuple[float, int]]
) -> float:
    """
    ...
    matches = sum(1 for i1, i2 in zip(intervals1, intervals2, strict=False) if i1 == i2)
...
def _detect_sections_automatically(
    composition: Composition, music21_stream: stream.Stream | None = None
) -> list[dict[str, Any]]:
    """
...
def generate_expansion_strategies(
    ...
    if motifs:
        strategies.append(
            f"Develop detected motifs ({len(motifs)} found) with variations and sequences"
        )
...
def analyze_composition(
    composition: Composition, music21_stream: stream.Stream | None = None, verbose: bool = False
) -> MusicalAnalysis:
    """
    ...
    if music21_stream is None:
        ...
        if verbose:
            print(
                f"    [Timing] Convert to music21 stream: {time.time() - stream_start:.2f}s",
                file=sys.stderr,
            )
    ...
    key_ideas = identify_key_ideas(
        composition, music21_stream=music21_stream, motifs=motifs, phrases=phrases
    )
    if verbose:
        print(
            f"    [Timing] Identify key ideas: {time.time() - key_ideas_start:.2f}s",
            file=sys.stderr,
        )
    ...
    if verbose:
        print(
            f"    [Timing] Generate expansion strategies: {time.time() - expansion_start:.2f}s",
            file=sys.stderr,
        )

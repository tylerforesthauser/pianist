        ...
        if MUSIC21_AVAILABLE:
            try:
                ...
                strategy = generate_expansion_strategy(
                    comp, args.target_length, analysis=musical_analysis
                )
                ...
                if strategy.motif_developments:
                    expansion_instructions += (
                        f"Motif Development ({len(strategy.motif_developments)} motifs):\n"
                    )
                ...
                if strategy.section_expansions:
                    expansion_instructions += (
                        f"Section Expansions ({len(strategy.section_expansions)} sections):\n"
                    )
                ...
                if (
                    musical_analysis.harmonic_progression
                    and musical_analysis.harmonic_progression.chords
                ):
                    ...
                    if musical_analysis.harmonic_progression.key:
                        expansion_instructions += (
                            f"Key: {musical_analysis.harmonic_progression.key}. "
                        )
                ...
                try:
                    ...
                    references = ref_db.find_relevant_references(
                        comp, analysis=musical_analysis, limit=3
                    )
                    if references:
                        ...
                        for ref in references:
                            ...
                            if ref.techniques:
                                expansion_instructions += (
                                    f" (Techniques: {', '.join(ref.techniques)})"
                                )
                            expansion_instructions += "\n"
                ...
                if args.verbose:
                    sys.stderr.write(
                        f"warning: Musical analysis failed: {e}. Using basic expansion instructions.\n"
                    )
        ...
        if raw_text is None:
            ...
            try:
                raw_text = generate_text_unified(
                    provider=provider, model=model, prompt=prompt, verbose=args.verbose
                )
            ...
            if MUSIC21_AVAILABLE:
                try:
                    validation_result = validate_expansion(
                        comp, expanded_comp, target_length=args.target_length
                    )
                    ...
                    if validation_result.passed:
                        ...
                        sys.stderr.write("❌ Validation failed. Review issues above.\n")
                except Exception as e:
                    if args.debug:
                        traceback.print_exc(file=sys.stderr)
                    sys.stderr.write(
                        f"warning: Validation failed: {e}. Proceeding with basic checks.\n"
                    )
                ...
                sys.stderr.write(
                    "warning: Validation requires music21. Install with: pip install music21\n"
                )
...
def setup_parser(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Model name to use with the provider. Defaults to config file or provider default. Only used with --provider.",
    )
    parser.add_argument(
        "--preserve-motifs",
        action="store_true",

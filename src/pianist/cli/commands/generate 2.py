...
def handle_generate(args) -> int:
    ...
    try:
        ...
        if not description:
            raise ValueError(
                "Composition description is required. Provide as argument or pipe text to stdin."
            )
        ...
        if args.render and provider is None:
            raise ValueError("--render requires --provider or config file to specify AI provider.")
        ...
        if args.render:
            ...
            out_midi_path = (
                resolve_output_path(args.out_midi_path, output_dir, "composition.mid", "generate")
                if args.out_midi_path is not None
                else None
            )
        prompt_out_path = (
            resolve_output_path(args.prompt_out_path, output_dir, "prompt.txt", "generate")
        ...
        # Generate composition using AI provider (provider already resolved above)
...
def setup_parser(parser):
    ...
    parser.add_argument(
        ...
        default=None,
        help=f"AI provider to use for generation: 'gemini' (cloud), 'ollama' (local), or 'openrouter' (cloud). Defaults to config file or '{get_ai_provider()}'.",
    )
    parser.add_argument(
        "--model",
        type=str,

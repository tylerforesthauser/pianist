...
from ...analyze import analysis_prompt_template, analyze_midi
...
def format_analysis_text(analysis_result: dict[str, Any]) -> str:
    ...
    if technical:
        ...
        if technical.get("duration_beats") is not None:
            lines.append(
                f"Duration: {technical['duration_beats']:.1f} beats ({technical.get('duration_seconds', 0):.1f} seconds, {technical.get('bars', 0):.1f} bars)"
            )
...
def handle_analyze(args) -> int:
    ...
    try:
        ...
        if args.render and args.provider:
            ...
            out_midi_path = (
                resolve_output_path(args.out_midi_path, output_dir, "composition.mid", "analyze")
                if args.out_midi_path is not None
                else None
            )
        prompt_out_path = (
            resolve_output_path(args.prompt_out_path, output_dir, "prompt.txt", "analyze")
        ...
        if args.provider:
            ...
            if raw_text is None:
                ...
                model = (
                    args.model or get_ai_model(args.provider) or get_default_model(args.provider)
                )

                try:
                    raw_text = generate_text_unified(
                        provider=args.provider, model=model, prompt=prompt, verbose=args.verbose
                    )
            ...
            elif custom_raw_path is not None:
            ...
            if args.format in ("prompt", "both"):
                ...
                if prompt_out_path is not None:
                    ...
                    if args.format == "prompt":
                        sys.stdout.write(str(prompt_out_path) + "\n")
                # If --out was provided and we're in prompt-only mode, treat it as prompt output.
                elif args.format == "prompt" and args.out_path is not None:
                    write_text(out_json_path, prompt)
                    sys.stdout.write(str(out_json_path) + "\n")
                else:
                    sys.stdout.write(prompt)

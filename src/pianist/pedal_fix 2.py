...
def fix_pedal_patterns(comp: Composition) -> Composition:
    ...
    for track in fixed.tracks:
        ...
        pedal_info: list[tuple[int, PedalEvent]] = [
            (i, ev) for i, ev in enumerate(events) if isinstance(ev, PedalEvent)
        ]
            ...
            if pedal.duration == 0:
                if pedal.value == 127:  # Press event (standard full pedal down)
                    ...
                    for j in range(i + 1, len(pedal_info)):
                        ...
                        if (
                            next_idx not in processed_indices
                            and next_pedal.duration == 0
                            and next_pedal.value == 0
                            and next_pedal.start >= pedal.start
                        ):

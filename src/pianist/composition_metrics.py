...
def _detect_phrases(notes: list[_NoteEvent], beats_per_bar: float = 4.0) -> list[tuple[float, float]]:
    """
    ...
    return phrases if phrases else [(notes[0].start_beats, notes[-1].start_beats + notes[-1].duration_beats)]


def _detect_motifs(notes: list[_NoteEvent], min_length: int = 3, max_length: int = 8) -> list[MotifPattern]:
    """
    ...
    pattern_positions: DefaultDict[tuple[tuple[int, ...], tuple[float, ...]], list[float]] = defaultdict(list)
    pattern_note_indices: DefaultDict[tuple[tuple[int, ...], tuple[float, ...]], list[int]] = defaultdict(list)
    ...
    for i in range(len(sections) - 1):
        ...
        for note in notes:
            if current_section.start_beats <= note.start_beats < current_section.end_beats:
                if last_note_in_section is None or note.start_beats > last_note_in_section.start_beats:
        ...
        if last_note_in_section and first_note_in_next:
            gap = max(0.0, first_note_in_next.start_beats - (last_note_in_section.start_beats + last_note_in_section.duration_beats))
...
def _analyze_variation(notes: list[_NoteEvent], phrases: list[tuple[float, float]]) -> VariationMetrics:
    ...
    phrase_variance = float(sum((x - sum(phrase_lengths) / len(phrase_lengths)) ** 2 for x in phrase_lengths) / len(phrase_lengths)) if phrase_lengths else 0.0
...
def analyze_composition_metrics(midi_path: str | Path) -> CompositionMetrics:
        ...
        for msg in track:
            ...
            if msg.type == "note_on" and int(msg.velocity) > 0:
                ...
                if pitch in active_notes:
                    start_tick, velocity, channel = active_notes.pop(pitch)
    ...
    for i in range(len(sections) - 1):
        ...
        for note in notes:
            if note.start_beats <= section_end and (last_note is None or note.start_beats > last_note.start_beats):
                last_note = note
        ...
        if last_note and first_note_next:
            ...
            transition_length = max(0.0, first_note_next.start_beats - (last_note.start_beats + last_note.duration_beats))
            transition_lengths.append(transition_length)
    
    mean_transition = float(sum(transition_lengths) / len(transition_lengths)) if transition_lengths else None
    median_transition = float(sorted(transition_lengths)[len(transition_lengths) // 2]) if transition_lengths else None
    ...
    return CompositionMetrics(
        ...
        phrase_length_distribution=_calculate_distribution(phrase_lengths) if phrase_lengths else None,
        sections=sections,

...
from .schema import Composition, Event, NoteEvent, PedalEvent, TempoEvent, SectionEvent
...
def diff_compositions(original: Composition, modified: Composition) -> CompositionDiff:
    ...
    diff.time_signature_changed = (
        original.time_signature.numerator != modified.time_signature.numerator or
        original.time_signature.denominator != modified.time_signature.denominator
    )
...
def format_diff_text(diff: CompositionDiff, show_preserved: bool = False) -> str:
    ...
    # Metadata changes
    if diff.title_changed or diff.bpm_changed or diff.time_signature_changed or \
       diff.key_signature_changed or diff.ppq_changed:
    ...
    for track_idx, track_diff_list in enumerate(diff.track_diffs):
        ...
        lines.append(f"Track {track_idx + 1} ({diff.original.tracks[track_idx].name if track_idx < len(diff.original.tracks) else diff.modified.tracks[track_idx].name}):")
...
def format_diff_markdown(diff: CompositionDiff) -> str:
    ...
    for track_idx, track_diff_list in enumerate(diff.track_diffs):
        ...
        track_name = diff.original.tracks[track_idx].name if track_idx < len(diff.original.tracks) else diff.modified.tracks[track_idx].name

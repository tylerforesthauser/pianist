...
def _expand_gradual_tempo_change(
    ...
    num_steps = max(1, round(duration / step_size))
...
def render_midi_mido(composition: Composition, out_path: str | Path) -> Path:
        ...
        for ev in track.events:
            if isinstance(ev, TempoEvent):
                if ev.start == 0:
                    ...
                    if (
                        ev.start_bpm is not None
                        and ev.end_bpm is not None
    ...
    conductor.append(mido.MetaMessage("set_tempo", tempo=mido.bpm2tempo(initial_bpm), time=0))

    if composition.key_signature:
        # Mido accepts values like "C", "Gm", "F#", "Bb", etc.
        conductor.append(mido.MetaMessage("key_signature", key=composition.key_signature, time=0))
    ...
    for track in composition.tracks:
        ...
        tr.append(
            mido.Message("program_change", program=track.program, channel=track.channel, time=0)
        )

"""
Quality assessment module for MIDI files.

This module provides quality checking functionality used by:
- CLI `analyze` command (via comprehensive_analysis)
- Batch review scripts (database curation)
- Standalone quality check script
"""
...
class QualityReport:
    ...
    def calculate_scores(self) -> None:
        ...
        technical_errors = sum(
            1 for i in self.issues if i.category == "technical" and i.severity == "error"
        )
        technical_warnings = sum(
            1 for i in self.issues if i.category == "technical" and i.severity == "warning"
        )
        technical_score = max(0.0, 1.0 - (technical_errors * 0.3) - (technical_warnings * 0.1))

        # Musical score
        musical_errors = sum(
            1 for i in self.issues if i.category == "musical" and i.severity == "error"
        )
        musical_warnings = sum(
            1 for i in self.issues if i.category == "musical" and i.severity == "warning"
        )
        musical_score = max(0.0, 1.0 - (musical_errors * 0.2) - (musical_warnings * 0.05))

        # Structure score
        structure_errors = sum(
            1 for i in self.issues if i.category == "structure" and i.severity == "error"
        )
        structure_warnings = sum(
            1 for i in self.issues if i.category == "structure" and i.severity == "warning"
        )
        ...
        self.overall_score = technical_score * 0.4 + musical_score * 0.4 + structure_score * 0.2
...
def check_musical_quality(
    composition: Any, report: QualityReport, musical_analysis: Any | None = None
) -> None:
    ...
    try:
        ...
        report.add_issue(
            QualityIssue("error", "musical", f"Musical analysis failed: {e}", {"error": str(e)})
        )
...
def check_structure_quality(midi_analysis: Any, report: QualityReport) -> None:
    ...
    if bars < 1:
        report.add_issue(
            QualityIssue("error", "structure", "File is too short (< 1 bar)", {"bars": bars})
        )
    elif bars < 4:
        report.add_issue(
            QualityIssue(
                "warning", "structure", f"Very short file ({bars:.1f} bars)", {"bars": bars}
            )
        )
...
Be concise and specific."""
        ...
        response = generate_text_unified(
            provider=provider, model=model, prompt=prompt, verbose=False
        )
        report.ai_assessment = response.strip()

    except (GeminiError, OllamaError, OpenRouterError) as e:
        report.add_issue(
            QualityIssue("warning", "technical", f"AI assessment failed: {e}", {"error": str(e)})
        )
    except Exception as e:
        report.add_issue(
            QualityIssue("warning", "technical", f"AI assessment failed: {e}", {"error": str(e)})
...
def check_midi_file(
    ...
    try:
        ...
        report.add_issue(
            QualityIssue(
                "error", "technical", f"Failed to analyze MIDI file: {e}", {"error": str(e)}
            )
        )
        report.calculate_scores()
...
def print_report(report: QualityReport, verbose: bool = False) -> None:
    ...
    # Overall score
    score_emoji = (
        "✅" if report.overall_score >= 0.8 else "⚠️" if report.overall_score >= 0.6 else "❌"
    )

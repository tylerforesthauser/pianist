...
4. **AI identification** - Use AI to identify well-known compositions
...
4. **AI identification** (if `--ai` flag used)
   - Attempts to identify well-known compositions
   - Only if filename doesn't have clear info
...
### Example 4: Unknown Work with AI

**File:** `unknown_modern.mid` (no JSON, no directory structure)

**Result:** Script uses AI identification (if `--ai` flag) or generates descriptive name from analysis.

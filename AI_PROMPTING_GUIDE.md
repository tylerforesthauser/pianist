...
- **Form**: Use classical and romantic formal structures as creative frameworks (see "Formal Structures" section below).
...
- **Length**: For substantial works, aim for 100-500+ beats, using formal structures to organize longer compositions.
...
Musical goals:
- Use a clear form (see Formal Structures section for options: binary, ternary, rondo, sonata form, theme & variations, dance forms, character pieces, or multi-movement works).
- For lengthier compositions (100+ beats), consider sonata form, extended variations, or multi-movement structures.
- Introduce a short motif early, then develop it (transpose/invert/augment/fragment/sequence) throughout.
- Use dynamics via velocity (p -> mf -> f and back) to shape phrases and larger formal sections.
- Mark formal sections using the `section` field (e.g., "exposition", "A", "development", "B") for clarity.
- Keep timing consistent: start/duration in beats; allow chords by multiple pitches.
- Create contrast between sections (different keys, textures, moods) and use transitions to connect them.
```

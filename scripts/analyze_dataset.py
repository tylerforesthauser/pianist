#!/usr/bin/env python3
"""
Batch analysis tool for extracting composition metrics from a dataset of professional MIDI files.

Usage:
    python analyze_dataset.py <midi_directory> [--output output.json]
...
def aggregate_metrics(metrics_list: list[CompositionMetrics]) -> dict[str, Any]:
    ...
    motif_frequencies = [m.motif_frequency_per_64_beats for m in valid_metrics if m.motif_frequency_per_64_beats is not None]
    if motif_frequencies:
        aggregated["motif_frequency"] = {
            ...
            "p25": float(statistics.quantiles(motif_frequencies, n=4)[0]) if len(motif_frequencies) >= 4 else None,
            "p75": float(statistics.quantiles(motif_frequencies, n=4)[2]) if len(motif_frequencies) >= 4 else None,
        }
    ...
    if section_gaps:
        aggregated["section_gaps"] = {
            ...
            "p95": float(statistics.quantiles(section_gaps, n=20)[18]) if len(section_gaps) >= 20 else None,
        }
    ...
    if all_transitions:
        aggregated["transition_lengths"] = {
            ...
            "p25": float(statistics.quantiles(all_transitions, n=4)[0]) if len(all_transitions) >= 4 else None,
            "p75": float(statistics.quantiles(all_transitions, n=4)[2]) if len(all_transitions) >= 4 else None,
        }
    
    # Variation metrics
    rhythmic_entropies = [m.variation_metrics.rhythmic_entropy for m in valid_metrics if m.variation_metrics]
    melodic_entropies = [m.variation_metrics.melodic_entropy for m in valid_metrics if m.variation_metrics]
    phrase_variances = [m.variation_metrics.phrase_length_variance for m in valid_metrics if m.variation_metrics]
    ...
    if all_phrase_lengths:
        aggregated["phrase_lengths"] = {
            ...
            "common_lengths": dict(Counter(round(pl, 1) for pl in all_phrase_lengths).most_common(10)),
        }
    ...
    if short_pieces:
        short_motif_freqs = [m.motif_frequency_per_64_beats for m in short_pieces if m.motif_frequency_per_64_beats]
    ...
    if medium_pieces:
        medium_motif_freqs = [m.motif_frequency_per_64_beats for m in medium_pieces if m.motif_frequency_per_64_beats]
    ...
    if long_pieces:
        long_motif_freqs = [m.motif_frequency_per_64_beats for m in long_pieces if m.motif_frequency_per_64_beats]
        if long_motif_freqs:
            if "by_duration" not in aggregated:
                aggregated["by_duration"] = {}

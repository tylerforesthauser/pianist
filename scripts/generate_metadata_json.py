...
def generate_metadata_interactive(midi_path: Path, existing: dict[str, any] | None = None) -> dict[str, any]:
    ...
    metadata["source"] = prompt_for_value(
        "Source (e.g., 'Public domain', 'IMSLP', etc.) - Optional",
...
def generate_metadata_from_args(args: argparse.Namespace, existing: dict[str, any] | None = None) -> dict[str, any]:
...
Examples:
  ...
  python scripts/generate_metadata_json.py --template
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help="MIDI file to generate metadata for"
    )
    ...
    parser.add_argument(
        "--template",
        action="store_true",
        help="Show metadata template/example and exit"
    )

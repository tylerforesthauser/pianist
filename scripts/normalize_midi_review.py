...
def parse_similarity_scores(similarity_scores_str: str) -> dict[str, float]:
    ...
    if not similarity_scores_str or not similarity_scores_str.strip():
        return scores
    for pair in similarity_scores_str.split(';'):
        pair = pair.strip()
        if ':' in pair:
...
def build_duplicate_groups(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    for filename in filename_to_row.keys():
...
def validate_data_completeness(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    all_fields = [
        'filename', 'filepath', 'quality_score', 'quality_issues',
        'suggested_name', 'suggested_id', 'suggested_style', 'suggested_description',
        'detected_key', 'detected_form', 'duration_beats', 'duration_seconds',
        'bars', 'tempo_bpm', 'time_signature', 'key_signature', 'tracks',
        'motif_count', 'phrase_count', 'chord_count', 'harmonic_progression',
        'is_duplicate', 'duplicate_group', 'similar_files', 'similarity_scores',
        'technical_score', 'musical_score', 'structure_score', 'is_original',
    ]
...
def main() -> int:
    ...
    parser.add_argument('--output-dir', type=Path, default=None, help='Directory for renamed files (default: same as --dir)')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be done without making changes')
    parser.add_argument('--min-quality', type=float, default=0.7, help='Minimum quality score (default: 0.7)')
    parser.add_argument('--min-technical', type=float, default=0.5, help='Minimum technical score (default: 0.5)')
    parser.add_argument('--min-musical', type=float, default=0.5, help='Minimum musical score (default: 0.5)')
    parser.add_argument('--min-structure', type=float, default=0.5, help='Minimum structure score (default: 0.5)')
    parser.add_argument('--reports-dir', type=Path, default=None, help='Directory for output reports (default: same directory as CSV)')
    parser.add_argument('--include-originals', action='store_true', help='Include original compositions in import metadata (default: exclude)')
    parser.add_argument('--include-excluded', action='store_true', help='Include files flagged for exclusion in import metadata (default: exclude)')
    ...
    metadata_list = [extract_metadata(row) for row in rows]
    ...
    # Identify exclusions
    print("\n=== Exclusion Analysis ===")
    exclusions = identify_exclusions(rows, args.min_quality, args.min_technical, args.min_musical, args.min_structure)
    ...
    with open(duplicate_csv_path, 'w', encoding='utf-8', newline='') as f:
        fieldnames = [
            'group_id', 'filename', 'suggested_name', 'suggested_style',
            'quality_score', 'technical_score', 'musical_score', 'structure_score',
            'detected_key', 'detected_form', 'duration_beats', 'bars',
            'tempo_bpm', 'time_signature', 'motif_count', 'phrase_count', 'chord_count',
            'is_original', 'similarity_scores'
        ]
        ...
        for group_id, group_files in duplicate_groups.items():
            for file_data in group_files:
                ...
                similarity_str = '; '.join(f"{f}: {s:.3f}" for f, s in meta['similarity_scores'].items())
                writer.writerow({
                    'group_id': group_id,
    ...
    with open(exclusions_csv_path, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'filename', 'suggested_name', 'quality_score',
            'technical_score', 'musical_score', 'structure_score',
            'is_original', 'reasons'
        ])
    ...
    with open(rename_csv_path, 'w', encoding='utf-8', newline='') as f:
        ...
        for mapping in rename_mapping:
            writer.writerow([mapping['original'], mapping['new'], mapping['suggested_name'], mapping['suggested_id']])
    print(f"  Rename mapping CSV: {rename_csv_path}")
    
    # 6. Generate metadata CSV for batch import (ready for reference database)
    print(f"\n=== Generating Database Import Metadata ===")
    
    # Filter files for import
    excluded_filenames = {excl['filename'] for excl in exclusions} if not args.include_excluded else set()
    ...
    with open(import_metadata_path, 'w', encoding='utf-8', newline='') as f:
        ...
        fieldnames = [
            ...
            'detected_key', 'tempo_bpm', 'duration_beats', 'quality_score',
            'technical_score', 'musical_score', 'structure_score',
            'motif_count', 'phrase_count', 'chord_count', 'harmonic_progression',
            'time_signature', 'bars'
        ]
        ...
        for row in files_for_import:
            ...
            writer.writerow({
                'filename': filename,
                'id': meta['suggested_id'] or meta['filename'].replace('.mid', '').replace('.midi', ''),
                'title': meta['suggested_name'] or meta['filename'].replace('.mid', '').replace('.midi', ''),
                'description': meta['suggested_description'] or meta['suggested_name'] or 'Musical composition',
                ...
                'tempo_bpm': f"{meta['tempo_bpm']:.1f}" if meta['tempo_bpm'] else '',
                'duration_beats': f"{meta['duration_beats']:.1f}" if meta['duration_beats'] else '',
                'quality_score': f"{meta['quality_score']:.3f}" if meta['quality_score'] else '',
                'technical_score': f"{meta['technical_score']:.3f}" if meta['technical_score'] else '',
                'musical_score': f"{meta['musical_score']:.3f}" if meta['musical_score'] else '',
                'structure_score': f"{meta['structure_score']:.3f}" if meta['structure_score'] else '',
